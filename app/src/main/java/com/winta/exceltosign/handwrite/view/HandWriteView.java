package com.winta.exceltosign.handwrite.view;
...
import com.winta.exceltosign.handwrite.glide.SignFileOutputStream;
import com.winta.exceltosign.handwrite.view.point.DrawPoint;
import com.winta.exceltosign.handwrite.view.point.PointUtil;
import com.winta.exceltosign.handwrite.view.point.TimedPoint;
...
public class HandWriteView extends View {
    ...
    public void save(String path,String projectName) throws IOException {
        save(path,projectName, false, 0, false);
    }

    public void save(String path,String projectName, boolean isEncrypt) throws IOException {
        save(path,projectName, false, 0, isEncrypt);
    }
     ...
     * @param projectName 项目的名字，用来创建文件夹，和储存签名照片
     */
    public void save(String path,String projectName, boolean clearBlank, int blank, boolean isEncrypt) throws IOException {
        ...
        if (buffer != null) {
            //生成项目文件夹
            File filepath = new File(getContext().getFilesDir().getAbsolutePath()+projectName+"/");
            try {
                if (!filepath.exists()) {
                    filepath.mkdir();
                }
